...
if (isset($_GET['delete'])) {
    ...
    $get_teacher = mysqli_query($data, "SELECT email FROM teacher WHERE id=$id");

    if ($get_teacher && $teacher_row = mysqli_fetch_assoc($get_teacher)) {
        $teacher_email = mysqli_real_escape_string($data, $teacher_row['email']);

        mysqli_begin_transaction($data);

        $delete_teacher = mysqli_query($data, "DELETE FROM teacher WHERE id=$id");
        $delete_user = $delete_teacher ? mysqli_query($data, "DELETE FROM user WHERE email='$teacher_email' AND usertype='teacher'") : false;
        ...
        if ($delete_teacher && $delete_user) {
            ...
            $message = "❌ Error deleting teacher: " . mysqli_error($data);
            $message_type = "error";
        }
    } else {
    ...
    header("Location: view_teacher.php?msg=" . urlencode($message) . "&type=" . $message_type);
    exit();
}
...
if (isset($_GET['toggle'])) {
    ...
    $current = mysqli_query($data, "SELECT is_active FROM teacher WHERE id=$id");
    if ($current && $row = mysqli_fetch_assoc($current)) {
        $newStatus = ($row['is_active'] == 1) ? 0 : 1;
        mysqli_query($data, "UPDATE teacher SET is_active=$newStatus WHERE id=$id");
...
if ($search_term !== '') {
    $where[] = "(name LIKE '%$search_term%' OR email LIKE '%$search_term%' OR mobile LIKE '%$search_term%' OR branch LIKE '%$search_term%')";
}
$where_sql = count($where) ? ('WHERE ' . implode(' AND ', $where)) : '';

/* FETCH TEACHERS */
$result = mysqli_query($data, "SELECT * FROM teacher $where_sql ORDER BY id DESC");

/* FILTER OPTIONS - Fix NULL branches */
$branches = mysqli_query($data, "SELECT DISTINCT branch FROM teacher WHERE branch IS NOT NULL AND branch != '' ORDER BY branch");
...
// Reset result pointer for stats calculation
$temp_result = mysqli_query($data, "SELECT * FROM teacher");
if ($temp_result) {
    while ($row = mysqli_fetch_assoc($temp_result)) {
        if ($row['is_active'] == 1) $active_teachers++;
        $branch = $row['branch'] ?: 'All Branches';
        $branch_counts[$branch] = ($branch_counts[$branch] ?? 0) + 1;
    }
}
$unique_branches = count($branch_counts);
...
<head>
    ...
    <title>View Teachers | Admin Panel - StudyBuddyHub</title>
    ...
    <style>
        * {
        ...
        .modal-edit .modal-content-custom {
            max-width: 600px;
        }
        .form-group {
            margin-bottom: 1rem;
...
<div class="admin-wrap">
    ...
    <div class="sidebar" id="sidebar">
        ...
        <div class="sidebar-menu">
            ...
            <a href="view_teacher.php" class="active"> <i class="fas fa-user-tie"></i> <span>View Teachers</span> </a>
    ...
    <div class="main">
        ...
        <div class="modern-topbar">
            <div class="topbar-left">
                ...
                <div class="page-title">
                    <i class="fas fa-user-tie"></i> View Teachers
                </div>
            </div>
        ...
        <div class="content">
            <div class="page-header" data-aos="fade-up">
                <h2> <i class="fas fa-chalkboard-teacher" style="color: var(--primary);"></i> Teacher List </h2>
            ...
            <div class="filter-card" data-aos="fade-up" data-aos-delay="50">
                <form method="get" action="view_teacher.php" id="filterForm">
                    <div class="row g-3 align-items-end">
                        <div class="col-md-3">
                            <label class="form-label"><i class="fas fa-code-branch"></i> Branch</label>
                            <select name="branch" class="form-select">
                                <option value="">All Branches</option>
                                <option value="NULL" <?php echo ($branch_filter === 'NULL') ? 'selected' : ''; ?>>All Branches (No Specific)</option>
                                <?php 
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" name="search" class="form-control" placeholder="Name, Email, Mobile, Branch..." value="<?php echo htmlspecialchars($search_term); ?>">
                        </div>
                        <div class="col-md-2">
                            <div class="d-flex gap-2">
                                <button type="submit" class="btn-filter w-100"><i class="fas fa-filter me-1"></i>Filter</button>
                                <a href="view_teacher.php" class="btn-reset"><i class="fas fa-undo-alt"></i></a>
                            </div>
                        </div>
                    </div>
                </form>
                ...
                <div class="table-header">
                    ...
                    <div class="d-flex gap-2">
                        <div class="total-count">
                            <i class="fas fa-user-tie"></i> Total: <?php echo $total_teachers; ?> Teachers
                        </div>
                        <button class="btn-add" onclick="openInQuickModal('add_teacher.php', 'Add New Teacher', 'fa-user-plus')">
                    ...
                    <?php if ($total_teachers > 0): ?>
                        <table id="teachersTable" class="display nowrap" style="width:100%">
                            ...
                            <tbody>
                                ...
                                <tr id="teacher-row-<?php echo $row['id']; ?>">
                                        ...
                                        <div class="teacher-info">
                                            <div class="teacher-avatar">
                                                <?php if (!empty($row['profile_pic']) && file_exists(__DIR__ . '/../uploads/profile_pics/' . $row['profile_pic'])): ?>
                                                    <img src="../uploads/profile_pics/<?php echo htmlspecialchars($row['profile_pic']); ?>" alt="Avatar" style="width: 100%; height: 100%; border-radius: 50%; object-fit: cover;">
                                                <?php else: ?>
                                    ...
                                    <td>
                                        <span class="branch-badge">
                                            <i class="fas fa-phone-alt"></i> <?php echo htmlspecialchars($row['mobile']); ?>
                                        </span>
                                    </td>
                                    <td>
                                        <?php if ($row['branch']): ?>
                                        ...
                                        <?php else: ?>
                                            <span class="branch-badge">All Branches</span>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="action-btns">
                                            <button class="btn-toggle" onclick="toggleTeacher(<?php echo $row['id']; ?>, <?php echo $row['is_active']; ?>)">
                                                <i class="fas fa-power-off"></i> Toggle
                                            </button>
                                            <button class="btn-assign-subject" onclick="openInQuickModal('assign_subjects.php?teacher_id=<?php echo $row['id']; ?>', 'Assign Subject to <?php echo htmlspecialchars($row['name']); ?>', 'fa-book')">
                                                <i class="fas fa-book"></i> Assign Subject
                                            </button>
                                            <button class="btn-edit" onclick='openEditModal({id:<?php echo $row['id']; ?>, name:"<?php echo addslashes($row['name']); ?>", email:"<?php echo addslashes($row['email']); ?>", mobile:"<?php echo addslashes($row['mobile']); ?>", branch:"<?php echo addslashes($row['branch']); ?>", qualification:"<?php echo addslashes($row['qualification']); ?>", experience:"<?php echo addslashes($row['experience']); ?>", address:"<?php echo addslashes($row['address']); ?>", is_active:<?php echo $row['is_active']; ?>})'>
                                                <i class="fas fa-edit"></i> Edit
                                            </button>
                                            <button class="btn-delete" onclick="confirmDelete(<?php echo $row['id']; ?>, '<?php echo addslashes($row['name']); ?>')">
                                                <i class="fas fa-trash-alt"></i> Delete
                                            </button>
                                            <button class="btn-view" onclick='viewDetails(<?php echo json_encode($row); ?>)'>
            ...
            <div class="stats-grid" data-aos="fade-up" data-aos-delay="150">
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-info">
                        <p>Branches</p>
                        <h3><?php echo $unique_branches; ?></h3>
                    </div>
                </div>
            </div>
            <?php endif; ?>
...
<div class="modal-custom" id="viewModal">
    <div class="modal-content-custom">
        <div class="modal-header-custom">
            <h5><i class="fas fa-chalkboard-teacher"></i> Teacher Details</h5>
...
<div class="modal-edit" id="editModal">
    <div class="modal-content-custom">
        <div class="modal-header-custom">
            <h5><i class="fas fa-edit"></i> Edit Teacher</h5>
            <button class="modal-close" onclick="closeEditModal()">&times;</button>
        </div>
        <form method="post" id="editTeacherForm" action="view_teacher.php">
            ...
            <div class="modal-body-custom">
                ...
                <div class="form-group">
                    ...
                    <select name="branch" id="edit_branch" class="form-control">
                        <option value="">All Branches (No Specific)</option>
                        <?php
                        $all_branches = mysqli_query($data, "SELECT branch_code, branch_name FROM branches WHERE is_active = 1 ORDER BY branch_code");
                        while ($b = mysqli_fetch_assoc($all_branches)) {
                            echo '<option value="' . $b['branch_code'] . '">' . $b['branch_code'] . ' - ' . $b['branch_name'] . '</option>';
                        }
                        ?>
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="experience" id="edit_experience" class="form-control" min="0" max="50">
                </div>
                <div class="form-group">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn-save"><i class="fas fa-save"></i> Update Teacher</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    // DataTable - Fixed with correct column count
    <?php if ($total_teachers > 0): ?>
    $(document).ready(function() {
        ...
        $('#teachersTable').DataTable({
            ...
            columnDefs: [
                ...
                { targets: 6, orderable: false, width: '250px' }
            ],
            language: { 
                search: "Search:", 
    ...
    // Toggle Teacher Status with SweetAlert
    function toggleTeacher(id, currentStatus) {
        const newStatus = currentStatus == 1 ? 'deactivate' : 'activate';
        Swal.fire({
            title: `${newStatus === 'activate' ? 'Activate' : 'Deactivate'} Teacher?`,
            text: `Are you sure you want to ${newStatus} this teacher?`,
            icon: 'question',
            ...
            if (result.isConfirmed) {
                window.location.href = `view_teacher.php?toggle=${id}`;
            }
        });
    }
    ...
    function confirmDelete(id, teacherName) {
        Swal.fire({
            ...
            html: `Are you sure you want to delete <strong>${escapeHtml(teacherName)}</strong>?<br><br>This will also remove their login access. This action cannot be undone.`,
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                window.location.href = `view_teacher.php?delete=${id}`;
            }
        });
    }

    // View Details Function
    function viewDetails(teacher) {
        let avatarHtml = '';
        if (teacher.profile_pic && teacher.profile_pic !== '') {
            avatarHtml = `<img src="../uploads/profile_pics/${teacher.profile_pic}" style="width: 60px; height: 60px; border-radius: 50%; object-fit: cover; margin-bottom: 10px; border: 2px solid var(--primary);">`;
        } else {
            avatarHtml = `<div style="width: 60px; height: 60px; border-radius: 16px; background: linear-gradient(135deg, var(--primary), var(--primary-dark)); margin: 0 auto 10px; display: flex; align-items: center; justify-content: center; color: white; font-weight: bold; font-size: 1.5rem;">${teacher.name.charAt(0).toUpperCase()}</div>`;
        }
        
        let modalContent = `
            <div class="text-center mb-3">
                ${avatarHtml}
                <h5 style="font-weight: 700; color: var(--dark);">${escapeHtml(teacher.name)}</h5>
                <p style="color: var(--gray); font-size: 0.8rem;">${escapeHtml(teacher.email)}</p>
            </div>
            ...
            <div class="detail-row">
                <div class="detail-label"><i class="fas fa-phone-alt"></i> Mobile Number</div>
                <div class="detail-value">${escapeHtml(teacher.mobile)}</div>
            </div>
            <div class="detail-row">
                <div class="detail-label"><i class="fas fa-code-branch"></i> Branch</div>
                <div class="detail-value">${escapeHtml(teacher.branch) || 'All Branches'}</div>
            </div>
            <div class="detail-row">
            ...
            ${teacher.qualification ? `<div class="detail-row">
                <div class="detail-label"><i class="fas fa-graduation-cap"></i> Qualification</div>
                <div class="detail-value">${escapeHtml(teacher.qualification)}</div>
            </div>` : ''}
            ${teacher.experience ? `<div class="detail-row">
                <div class="detail-label"><i class="fas fa-briefcase"></i> Experience</div>
                <div class="detail-value">${escapeHtml(teacher.experience)} Years</div>
            </div>` : ''}
            ${teacher.address ? `<div class="detail-row">
                <div class="detail-label"><i class="fas fa-map-marker-alt"></i> Address</div>
                <div class="detail-value">${escapeHtml(teacher.address)}</div>
            </div>` : ''}
        `;
    ...
    function openEditModal(teacher) {
        ...
        document.getElementById('edit_mobile').value = teacher.mobile;
        document.getElementById('edit_branch').value = teacher.branch || '';
        document.getElementById('edit_qualification').value = teacher.qualification || '';
        document.getElementById('edit_experience').value = teacher.experience || 0;
