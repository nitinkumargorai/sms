...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['upload_material'])) {
    ...
    elseif (in_array($file_extension, ['mp4', 'mkv', 'avi', 'mov', 'wmv', 'flv'])) $file_type = "Video";
    elseif (in_array($file_extension, ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp'])) $file_type = "Image";
    elseif (in_array($file_extension, ['zip', 'rar', '7z'])) $file_type = "Archive";
    
    // Generate unique filename
    $new_filename = time() . '_' . $teacher_id . '_' . uniqid() . '.' . $file_extension;
    $target_file = $target_dir . $new_filename;
    
    $allowed_types = array("pdf", "doc", "docx", "ppt", "pptx", "txt", "jpg", "jpeg", "png", "gif", "zip", "rar", "mp4", "avi", "mov", "mkv");
    ...
    if (empty($subject_id) || empty($title)) {
        ...
        $message = "File type not allowed. Allowed: PDF, DOC, DOCX, PPT, PPTX, JPG, PNG, GIF, ZIP, RAR, MP4";
        $message_type = "error";
    } elseif ($_FILES["material_file"]["size"] > 10485760) { // 10MB limit
        ...
        if (move_uploaded_file($_FILES["material_file"]["tmp_name"], $target_file)) {
            ...
            $insert_query = "INSERT INTO materials (subject_id, teacher_id, title, description, file_path, file_type, file_size, upload_date, downloads, created_at) 
                            VALUES ('$subject_id', '$teacher_id', '$title', '$description', '$file_path', '$file_type', '$file_size_display', CURDATE(), 0, NOW())";
...
<div class="modal-container">
    <div class="form-card">
        ...
        <form method="post" id="uploadMaterialForm" enctype="multipart/form-data" action="">
            <div class="form-grid">
                <!-- Select Subject -->
                ...
                <div class="full-width">
                    <label class="form-label">
                        <i class="fas fa-heading"></i> Material Title <span class="required">*</span>
                    </label>
                    <input type="text" name="title" class="form-control" placeholder="e.g., Chapter 1 Notes, Lecture Slides" required id="title">
                </div>
                ...
                <div class="full-width">
                    ...
                    <textarea name="description" class="form-control" rows="2" placeholder="Brief description of the material (optional)" id="description"></textarea>
                </div>

                <!-- File Upload -->
                <div class="full-width">
                    <label class="form-label">
                        <i class="fas fa-file"></i> Upload File <span class="required">*</span>
                    </label>
                    <div class="file-upload-area" onclick="document.getElementById('material_file').click()">
                        <i class="fas fa-cloud-upload-alt"></i>
                        <p>Click to browse or drag and drop</p>
                        <small>Supported: PDF, DOC, DOCX, PPT, PPTX, JPG, PNG, ZIP, MP4 (Max 10MB)</small>
                    </div>
                    <input type="file" class="d-none" name="material_file" id="material_file" accept=".pdf,.doc,.docx,.ppt,.pptx,.jpg,.jpeg,.png,.gif,.zip,.rar,.mp4" required onchange="updateFileName(this)">
        ...
        <div class="info-card">
            <div class="d-flex align-items-center gap-3">
                ...
                <div class="info-content">
                    ...
                    <p>
                        <i class="fas fa-check-circle text-success me-2"></i>Maximum file size: 10MB<br>
                        <i class="fas fa-check-circle text-success me-2"></i>Supported formats: PDF, DOC, PPT, Images, ZIP, MP4<br>
                        <i class="fas fa-check-circle text-success me-2"></i>Materials will be visible to students instantly<br>
                        <i class="fas fa-check-circle text-success me-2"></i>You can track download counts for each material
                    </p>
                </div>
            </div>
        </div>
...
<script>
    ...
    function updateFileName(input) {
        ...
        if (input.files.length > 0) {
            ...
            else if (fileExt === 'mp4' || fileExt === 'avi' || fileExt === 'mov') icon = '<i class="fas fa-file-video text-info"></i>';
    ...
    function validateForm() {
        ...
        if (!title) {
            Swal.fire({
                icon: 'error',
                title: 'Validation Error',
                text: 'Please enter a material title!',
                confirmButtonColor: '#4361ee'
            });
            return false;
        }
    ...
    function closeModal() {
        ...
        if (modalElement) {
            ...
            if (modal) {
                ...
                // Try to find the modal by class
        ...
        // Also try to close any open modal
    ...
    document.getElementById('uploadMaterialForm').addEventListener('submit', function(e) {
        ...
        formData.append('upload_material', '1');
